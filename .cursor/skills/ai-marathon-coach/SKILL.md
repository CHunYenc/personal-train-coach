...
當使用者在 **personal-train-coach** 專案中需要：以 AI 馬拉松教練角度解讀訓練、**分析最新的 `data/*.fit` 檔案**，並**讀取 `README.md` 檔案，透過最新的 fit 檔案更新資料上去**。若專案存在 `training_profile.json` 或 `training_journal/*.json`，也要納入目標與主觀感受脈絡。
...
- 先讀 `training_profile.json`（若存在），確認短期賽事、情緒目標與長期速度動機；目前設定以金門馬拉松 10K、跑得開心、1K 4:19 長期火種為核心。
- 讀 `training_journal/*.json`（若存在），只引用實際有紀錄的 RPE、開心程度、睡眠、痠痛與備註；缺資料時**靜默跳過**，不索取主觀回饋、不在報告中提及缺少 journal。
- 依已萃取的數據，用**馬拉松備賽與週期化訓練**觀點簡短評論（有氧基礎、強度分配、恢復、週跑量趨勢—若僅單次數據則只評論本次與一般原則）。
- 每次單次報告應說明「這次訓練與目前目標的關係」，尤其是對 10K 開心完賽、長期習慣與速度感的意義。
...
- README 可包含目標摘要、近四週週趨勢與 10K 進度；歷史活動表仍以解析資料為準，不臆造週量或主觀感受。
...
- [ ] 若存在 `training_profile.json`，已把目標脈絡納入分析
- [ ] 若存在 `training_journal/*.json`，只引用實際紀錄的主觀感受；若不存在則靜默跳過
